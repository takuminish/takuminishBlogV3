...
仕様書はdocsディレクトリ内に保存されている。ファイル一覧とステータスは `docs/README.md` を参照すること。

- `docs/specifications/` : サイトの機能仕様書（記事・スクラップなど）
- `docs/system-design/` : コード設計・アーキテクチャ・インフラの仕様書

作業を行う際は仕様書を作成し、進めること。実装や仕様を変更した場合は必ずdocsディレクトリの仕様書も修正すること。ドキュメントを追加・変更した場合は `docs/README.md` の一覧も更新すること。
